...
# get model predictions with confidence intervals using ggpredict
my_terms <- c("height_cm [all]", "ant_species")

predictions <- ggpredict(model2, terms = my_terms)
...
ggplot() +
  ...
  facet_wrap(~ group,
             labeller = labeller(group = c("AB"  = "C. sjostedti",
                                           "RRB" = "C. mimosae",
                                           "BBR" = "C. nigriceps"))) +
  theme_classic() +
...
#creating model prediction for 600cm
ggpredict(model2,
...
drinks_raw <- read.csv(here("data", "caffeinated_drinks.csv")) |>
  clean_names() |> 
